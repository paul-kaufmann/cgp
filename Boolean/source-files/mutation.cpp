...
int mutate_a_gene(int *chromosome, int *active_nodes, int *mutation_type) {
	...
	while (1) { // try mutating until success
		...
		if (is_not_output_gene(which_gene)) {
			if (is_function_gene(which_gene, which_locus)) {
				...
				{
					// seems o be an incorrect implementation, but we dont have the case of num_functions == 1
			...
			if (!active_nodes[num_inputs + num_nodes + (which_gene - (num_genes_per_node * num_nodes))] && !mutate_inactive_wires) {
				printf("primary output node is classified as inactive node. this should never happen\n");
				exit(1);
			}
	...
	printf("impossible situation occured: no gene found for mutation\n");
	exit(0);
	return (0);
}
...
/* ************************************************************
 ...
 implementation of the mutation operator flipping bits until nr_mutation active genes have been altered
 ...
 */
void get_used_nodes(const int *chromosome, int *nodes_used) {
	int i, j, index;
...
/**
 * convert from active nodes to active genes
 * the active_genes field should be pre-allocated
 */
void convert_active_nodes_to_genes(const int *active_nodes, int *active_genes) {
	// convert node indexing to gene indexing starting from first node and not from first primary input
	for (int i = 0; i < num_nodes * num_genes_per_node; i++)
		active_genes[i] = active_nodes[num_inputs + ((int) (i / num_genes_per_node))];
	for (int i = num_nodes * num_genes_per_node; i < num_nodes * num_genes_per_node + num_outputs; i++)
		active_genes[i] = 1; // primary outputs are always active connection genes
}

/**
 * compute active nodes and genes
 *
 * the active_nodes and active_genes fields should be preallocated
 */
void get_used_genes(const int *chromosome, int *active_nodes, int *active_genes) {
	get_used_nodes(chromosome, active_nodes);
	convert_active_nodes_to_genes(active_nodes, active_genes);
}

/**
 * compute active genes
 *
 * the active_genes field should be preallocated
 */
...
/** mutate exactly num_mutations genes in the genotype
 * regardless of active inactive status
 */
void mutate_chromosome(int *chromosome, int num_mutations, long long evaluation_no, optimization_log *log) {
	...
	while (i < num_mutations) {
		...
		// increase number of mutated genes only, if regular mutation in percentage of genetic material was required
		// or if G&P mutation is configured and an active gene has been mutated
		if ((genes_mutate == 0) || active_nodes[j])
			i++;
...
/** mutate chromosome according to the mutation rate
 * regardless of active inactive status
 */
//void mutate_chromosome(int *chromosome, long long evaluation_no, optimization_log *log) {
//	int num_mutations = get_num_mutant(num_genes, per_cent_mutate);
//	mutate_chromosome(chromosome, num_mutations, evaluation_no, log);
//}
...
// material mutated in the original mutation operator and this new one
	if (num_mutations < 1) {
		printf("num_mutations lower than 1: %d\n", num_mutations);
		exit(1);
	}
...
void mutate(int *chromosome, int evaluation_no, optimization_log *log) {
	...
	if (genes_mutate)
		// mutate genes_mutate active genes, Goldman and Punch
		mutate_chromosome(chromosome, genes_mutate, evaluation_no, log);
	else
		// mutate per_cent_mutate genes of a genotype, regardless of active/inactive status
		mutate_chromosome(chromosome, get_num_mutant(num_genes, per_cent_mutate), evaluation_no, log);
}
