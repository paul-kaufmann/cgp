	...
	*nodes_processed = 0;
	...
	do {
		...
		for (i = mu; i < mu + lambda; i++) {
			...
			//mutate_active_chromosome(population[i], 1, number_of_evaluations, &l);
			my_fitness[i] = fitness(population[i], nodes_processed);
			number_of_evaluations++;
		}
...
double hc(long long *evals_to_perfect, int *num_nodes_active, long *nodes_processed) {
	...
	// keep count of successful mutation types
	static std::map<int, int*> mutations;

	int nodes_to_process[MAX_NUM_NODES];

	optimization_log *log = new optimization_log();
	...
	*nodes_processed = 0;
	...
	while (costs < perfect && I < max_number_evaluations) {
		...
		*log->getLastFQ() = child_costs;

		// set fitness change type in the log
		if (child_costs < costs)		// fitness worse
			*log->getLastImpact() = -1;
		else if (child_costs > costs) { // functional quality improving
			*log->getLastImpact() = 1;

			// add to histogram the type of successful mutation
			if (mutations.find((int) child_costs) == mutations.end()) {
				mutations[(int) child_costs] = (int*) malloc(3 * sizeof(int));
				memset(mutations[(int) child_costs], 0, 3 * sizeof(int));
			}
			mutations[(int) child_costs][*log->getLastMutationType()]++;

		} else
			*log->getLastImpact() = 0; // functional quality same as parent's
	...
	*num_nodes_active = get_nodes_to_process(parent, nodes_to_process);
	...
	// print histogram of mutation types
	printf("\nmutation types begin\n");
	for (int i = 0; i <= (int) perfect; i++) {
		if (mutations.find((int) i) == mutations.end())
			continue;
		printf("%d %d %d %d\n", i, mutations[i][0], mutations[i][1], mutations[i][2]);
	}
	printf("\nmutation types end\n");
...
double run_optimizer(int num_runs_total) {
	...
	for (run = 0; run < num_runs_total; run++)
		printf("%f ", min(1.0, (fitnesses[run] / perfect)));
